...
public class Individual  implements Serializable {
     ...
     *
     * The Timetable class is a bit overloaded. It knows both fixed information (the
     * courses that MUST be scheduled, the professors that MUST be given jobs, the
     * classrooms that DO exist) -- but it also understands how to parse and unpack
     * chromosomes which contain variable information (which professor teaches which
     * class and when?)
     *
     ...
     * randomly assigns timeslots, rooms, and professors to the chromosome for each
     * student group and module.
     *
     ...
     * case is a little tricky -- used in Chapter 4, this constructor will create a
     * valid chromosome for a list of cities for the TSP, by using each city once
     * and only once.
     *
     * If used in Chapter 5, however, this will create an utterly INVALID chromosome
     * for the class scheduler. So you should not use this constructor if you hope
     * to create a valid random individual. For that purpose, use the
     * Individual(Timetable) constructor, which will create a valid Individual from
     * the fixed information in the Timetable object.
     *
     * However, Chapter 5 still needs an Individual(int) constructor that creates an
     * Individual with a chromosome of a given size. It's used in the
     * crossoverPopulation method in order to initialize the offspring. The fact
     * that this creates an invalid Individual doesn't matter in this case, because
     * the crossover algorithm immediately rewrites the whole chromosome.
     *
     *
