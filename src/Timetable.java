...
public class Timetable {
    ...
    /**
     * "Clone" a timetable. We use this before evaluating a timetable so we have a
     * unique container for each set of classes created by "createClasses".
     * Truthfully, that's not entirely necessary (no big deal if we wipe out and
     * reuse the .classes property here), but Chapter 6 discusses multi-threading
     * for fitness calculations, and in order to do that we need separate objects so
     * that one thread doesn't step on another thread's toes. So this constructor
     * isn't _entirely_ necessary for Chapter 5, but you'll see it in action in
     * Chapter 6.
     *
     * @param cloneable
     */
